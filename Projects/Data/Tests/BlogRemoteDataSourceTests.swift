//
//  BlogRemoteDataSourceTests.swift
//  Data
//
...
final class BlogRemoteDataSourceTests: XCTestCase {

    private var client: NetworkClientStub!
    private var sut: BlogRemoteDataSource!

    override func setUp() {
        super.setUp()
        client = NetworkClientStub()
        sut = BlogRemoteDataSourceImpl(client: client)
    }
    ...
    func test_success_decodesDTO() async throws {
        ...
        let dto = try await sut.fetchBlogs(req)

        XCTAssertEqual(dto.documents.count, 1)
        XCTAssertEqual(dto.documents.first?.url, "https://ex.com/1")
