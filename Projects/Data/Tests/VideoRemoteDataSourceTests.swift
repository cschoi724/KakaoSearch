//
//  VideoRemoteDataSourceTests.swift
//  Data
//
...
final class VideoRemoteDataSourceTests: XCTestCase {

    private var client: NetworkClientStub!
    private var sut: VideoRemoteDataSource!

    override func setUp() {
        super.setUp()
        client = NetworkClientStub()
        sut = VideoRemoteDataSourceImpl(client: client)
    }
    ...
    func test_success_decodesDTO() async throws {
        ...
        let dto = try await sut.fetchVideos(req)

        XCTAssertEqual(dto.documents.count, 1)
        XCTAssertEqual(dto.documents.first?.url, "https://video.ex.com/watch?v=1")
    ...
    func test_nonKakaoBody_keepsNetworkStatusCode() async {
        client.setNextResponse(status: 403, data: #"{"oops":"not kakao"}"#.data(using: .utf8))
